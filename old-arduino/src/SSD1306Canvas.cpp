...
SSD1306Canvas::SSD1306Canvas(uint8_t w, uint8_t h)
    : Adafruit_GFX(w, h), buffer_{new uint8_t[buffer_size()]} {}

SSD1306Canvas::~SSD1306Canvas() = default;

bool SSD1306Canvas::get_pixel(uint8_t x, uint8_t y) const {
  if (x >= width() || y >= height()) {
    return false;
  }
...
void SSD1306Canvas::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void SSD1306Canvas::fillScreen(uint16_t color) {
  memset(buffer_.get(), color ? 0xff : 0, buffer_size());
}

void SSD1306Canvas::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
...
void SSD1306Canvas::drawFastHLine(int16_t x,
                                  int16_t y,
                                  int16_t w,
                                  uint16_t color) {
...
void SSD1306Canvas::drawFastRawVLine(int16_t x,
                                     int16_t y,
                                     int16_t h,
                                     uint16_t color) {
...
void SSD1306Canvas::drawFastRawHLine(int16_t x,
                                     int16_t y,
                                     int16_t w,
                                     uint16_t color) {
  const auto [idx, mask] = get_pixel_idx(x, y);
